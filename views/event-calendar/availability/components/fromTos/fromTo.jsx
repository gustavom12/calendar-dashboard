...
function FromTo({ i2, handleChange, fromTo, i, day, setDaysConfig, daysConfig }) {
  ...
  return (
    <>
      <div className={`fromtoInputs ${overlaps && 'overlaps'}`} key={i2}>
        <span className="name">{i2 === 0 && <FormattedMessage id={`weekday3letters${i}`} />}</span>
        <TimePicker
          ...
          <Button
            onClick={() => {
              setDaysConfig(val => ({
                ...
                [i]: {
                  ...val[i],
                  config: val[i].config.filter(el => el !== val[i].config[i2]),
                },
              }))
            }}
            style={{ width: 25, border: 'none', padding: 0 }}
      ...
      {overlaps && (
        <div className="text-danger text-center w-100" style={{ marginTop: -3 }}>
